...
// NOTE: This test is just an extension of the 'SequencerInboxDelayBufferable' and
//       the 'SequencerInboxDelayBufferableBlobMock' tests but instead uses the EigenDA
//       inbox entrypoint with certificates generated arbitrarily for submission.
//       no certs are actually being verified since we use the L2 dummy blob verifier
describe('SequencerInboxDelayBufferableEigenDAMock', async () => {
  it('can deplete buffer', async () => {
    const { bridge, sequencerInbox, batchPoster, delayConfig, maxDelay } =
    ...
    const cert = await generateEigenDACertificate()
  ...
  it('can replenish buffer', async () => {
    ...
    await (
      await sequencerInbox
        .connect(batchPoster)
        .addSequencerL2BatchFromEigenDADelayProof(
          3,
          cert,
          ethers.constants.AddressZero,
          delayedMessageCount.add(1),
